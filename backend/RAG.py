...
from langchain.schema import HumanMessage, AIMessage  # Import the required message classes
import torchvision
...
def stream_llm_answer(query,img=None):
    if img!=None:
        imageOut = predict_image(img,model)    
        query = query+f".An image was passed to an image model and it classified it as {classes[imageOut]} assume the classifiers acuuracy is 100%. And dont mention the classifier in your answer."
...
while True:
    ...
    if inp == "7697":  # Exit condition
        ...
        stream_llm_answer(inp,img="C:\\banana\\codeables\\Projects\\SIH\\image.png")
